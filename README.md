...
- **Add log**: One button to record timestamped **seizures** or **notes** (backdating supported).
- **Scheduling**:
  - Daily
...
- **Restart ended meds**: Ended courses are shown in a collapsible section with a **Restart** action (clears `end_date`).
- **Course history**: Track multiple start/end windows for repeat courses/rescue meds (stored separately from the active `start_date`/`end_date` used on the tracker).
- **Per-medication history**: View dose events and check/uncheck logs for a single medication (`MedicationHistoryModal`).
- **History report** (`/history`): Filter by date range and medication; combines slot-based logs, flexible-dose events, seizures, and notes. CSV export included.
...
- Use **Add log** to record seizures or timestamped notes/measurements (you can backdate the time).
...
| `medication_course_periods` | Course history: multiple start/end windows for repeat courses/rescue meds |
...
| `symptom_events` | Symptom tracking (e.g. seizures) |
| `timeline_events` | Timestamped notes/visits/measurements (backdateable) |
