...
export default function Notices({ medications, selectedDate, selectedTimeSlot }: NoticesProps) {
  ...
  const summaryLineSingle = first
    ? `${first.name} not due on this date (next due: ${formatNextDue(nextDueDate(first, selectedDate))}).`
    : 'Medication not due on this date.';
  const summaryLineMulti = first
    ? `${first.name} not due on this date (next due: ${formatNextDue(nextDueDate(first, selectedDate))}) — and ${count - 1} more.`
    ...
    <div className="mx-2 sm:mx-3 mt-2 mb-2">
      {count === 1 ? (
        <div className="bg-red-100 border border-gray-300 rounded-lg p-2 sm:p-3 text-gray-900 text-xs sm:text-sm">
          ...
          <div className="mt-0.5 break-words">{summaryLineSingle}</div>
        </div>
      ) : (
